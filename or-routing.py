"""circuit soldering optimization"""
...
def euclidean_distance_calculation(location):
    dis = [[]] * len(location)
    for i in range(len(location)):
        dis[i] = [0 for x in range(len(location))]
        for j in range(len(location)):
            dx = location[i][0]-location[j][0]
            dy = location[i][1]-location[j][1]
            dis[i][j] = int(math.sqrt(dx**2 + dy**2))
    return dis
...
def main():
    ...
    distance = euclidean_distance_calculation(data['locations'])

    # Create the routing index manager.
    manager = pywrapcp.RoutingIndexManager(len(distance),
    ...
    #RegisterTransitCallback will return the solver's internal indices
    #Then distance_callback will convert this indcies to distance by using manager.IndextoNode
