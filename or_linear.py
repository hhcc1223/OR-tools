...
class or_linear_p:
    ...
    #minimum the sum of the food and satisfy the nutrient requirement
    def stigler_diet(self):
        #food list
        foodlist = [
        ...
        #Linear solver
        solver = pywraplp.Solver('stiglerfood',pywraplp.Solver.GLOP_LINEAR_PROGRAMMING)
        
        #Variables
        v = {}
        for i in range(len(foodlist)):
            v['_{}'.format(i)] = solver.NumVar(0,solver.infinity(),foodlist[i][0])
        
        #constrains
        ct = {}
        for i in range(len(nutrients_req)):
            ct['_{}'.format(i)] = solver.Constraint(nutrients_req[i][1],solver.infinity())
            for a in range(len(v)):
                ct['_{}'.format(i)].SetCoefficient(v['_{}'.format(a)],foodlist[a][i+3])
        
        #Object
        objective = solver.Objective()
        for i in range(len(v)):
            objective.SetCoefficient(v['_{}'.format(i)], 1)
        ...
        price = 0
        for i in range(len(v)):
            if v['_{}'.format(i)].solution_value() != 0:
                print(str(v['_{}'.format(i)])+": %f" %(v['_{}'.format(i)].solution_value()))
                price += v['_{}'.format(i)].solution_value()
        print("Annual cost = %f" %(price*365))
